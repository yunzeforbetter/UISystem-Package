...
namespace UISystem
{
    ...
    public class UIListView : UIScrollRect 
    {
        ...
        protected Dictionary<int, RectTransform> m_CellRTDict = new();                      //index-Cell字典    
        protected Stack<RectTransform> m_UnUseCellRTStack = new();                          //空闲Cell堆栈
        protected List<KeyValuePair<int, RectTransform>> m_CellRTListForSort = new();       //Cell列表用于辅助Sbling排序

        protected List<int> m_OldIndexes = new();                                           //旧的索引集合
        protected List<int> m_NewIndexes = new();                                           //新的索引集合
        protected List<int> m_AppearIndexes = new();                                        //将要出现的索引集合   //使用List而非单个，可以支持Content位置跳变
        protected List<int> m_DisAppearIndexes = new();                                     //将要消失的索引集合   //使用List而非单个，可以支持Content位置跳变
        protected List<int> m_StayIndexes = new();                                          //保持的索引集合       //使用List而非单个，可以支持Content位置跳变

        [SerializeField] protected Rect m_CellRect;                      //Cell Rect（必需）
        public Rect CellRect { get { return m_CellRect; } set { SetProperty(ref m_CellRect, value); } }

        [SerializeField] protected Vector2 m_CellPivot;                    //Cell中心点（必需）           
        public Vector2 CellPivot { get { return m_CellPivot; } set { SetProperty(ref m_CellPivot, value); } }      
        ...
        {
            this.CellRect = templateCellRT.rect;
            this.CellPivot = templateCellRT.pivot;
        ...
        public void Init(Func<int, int, RectTransform> onCreateCell, Action<int, int> onShowCell)
        {
            this.m_OnCreateCell = onCreateCell;
            this.m_OnShowCell = onShowCell;
        }
        ...
        protected void CalculateRequiredSpace()
        {
            if (m_CellCount > 0)
            {
                this.m_RequiredSpace = new Vector2(
                    m_ActualCellCountX * CellRect.size.x + (m_ActualCellCountX - 1) * spacing.x,
                    m_ActualCellCountY * CellRect.size.y + (m_ActualCellCountY - 1) * spacing.y
                );
            }
            else
            {
        ...
        protected void CalcIndexes()
        {
            ...
            if (m_MovementAxis == MovementAxis.Horizontal)
            {
                ...
                outCountFromStart = Mathf.FloorToInt((outWidthFromStart - startPadding + spacing.x) / (CellRect.size.x + spacing.x));
                //完全滑出右边界的数量（可为负），要向下取整，即尽量认为其没滑出，以保证可视区域内的正确性。（可为负）
                outCountFromEnd = Mathf.FloorToInt((outWidthFromEnd - endPadding + spacing.x) / (CellRect.size.x + spacing.x));
            }
            else
            {
                float contentStartPosY = -m_CellStartOffsetOnMovementAxis;
                ...
                outCountFromStart = Mathf.FloorToInt((outHeightFromStart - startPadding + spacing.y) / (CellRect.size.y + spacing.y));
                //完全滑出下边界的数量（可为负），要向下取整，即尽量认为其没滑出，以保证可视区域内的正确性。（可为负）
                outCountFromEnd = Mathf.FloorToInt((outHeightFromEnd - endPadding + spacing.y) / (CellRect.size.y + spacing.y));
            }
        ...
        protected Vector2 GetCellPos(int posIndex)
        {
            ...
            float anchoredPosX = (m_CellStartOffset.x + (CellRect.size.x + spacing.x) * posIndexX) + CellRect.size.x * CellPivot.x * scaleFactor.x;

            // y轴：-初始位置-宽度*(1-中心点偏移)*缩放系数 (y轴是向负方向)(从左上到右下)
            float anchoredPosY = -(m_CellStartOffset.y + (CellRect.size.y + spacing.y) * posIndexY) - CellRect.size.y * (1f - CellPivot.y) * scaleFactor.y;
        ...
        protected RectTransform GetOrCreateCell(int posIndex, int dataIndex)
        {
            ...
            else
            {
                ...
                cellRT.sizeDelta = CellRect.size;
            }
