    ...
    public class UIPageView : UIListView
    {
        ...
        protected override void FixPadding()
        {
            ...
            else
            {
                ...
                {
                    int fixedPaddingX = Mathf.FloorToInt((m_Viewport.rect.width - CellRect.width) / 2);
                    padding.left = padding.right = fixedPaddingX;
                }
                else
                {
                    int fixedPaddingY = Mathf.FloorToInt((m_Viewport.rect.height - CellRect.height) / 2);
        ...
        protected override void FixSpacing()
        {
            ...
            if (m_MovementAxis == MovementAxis.Horizontal)
            {
                float fixedSpacingX = m_Viewport.rect.width - CellRect.width;
                spacing = new Vector2(fixedSpacingX, spacing.y);
            }
            else
            {
                float fixedSpacingY = m_Viewport.rect.height - CellRect.height;
        ...
        private IEnumerator CarouselRoutine()
        {
            ...
            //页宽/高
            float pageSize = m_MovementAxis == MovementAxis.Horizontal ? CellRect.width + spacing.x : CellRect.height + spacing.y;
